    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == Intent.ACTION_POWER_CONNECTED ||
            ...
            kotlinx.coroutines.MainScope().launch {
                     ...
                     val repository = com.brittytino.patchwork.data.repository.SettingsRepository(context)
                     val hasPerm = com.brittytino.patchwork.utils.PermissionUtils.hasBluetoothPermission(context)
                     
                     val isEnabled = repository.isBluetoothDevicesEnabled()
                     val bluetoothDevices = if (isEnabled && hasPerm) {
                         com.brittytino.patchwork.utils.BluetoothBatteryUtils.getPairedDevicesBattery(context)
                     } else {
                         emptyList()
                     }
                     ...
                     glanceIds.forEach { glanceId ->
                         androidx.glance.appwidget.state.updateAppWidgetState(context, glanceId) { prefs ->
                            val KEY_SHOW = androidx.datastore.preferences.core.booleanPreferencesKey(com.brittytino.patchwork.data.repository.SettingsRepository.KEY_SHOW_BLUETOOTH_DEVICES)
                            val KEY_DATA = androidx.datastore.preferences.core.stringPreferencesKey(com.brittytino.patchwork.data.repository.SettingsRepository.KEY_BLUETOOTH_DEVICES_BATTERY)
                            val KEY_MAX = androidx.datastore.preferences.core.intPreferencesKey(com.brittytino.patchwork.data.repository.SettingsRepository.KEY_BATTERY_WIDGET_MAX_DEVICES)
                            val KEY_BG = androidx.datastore.preferences.core.booleanPreferencesKey(com.brittytino.patchwork.data.repository.SettingsRepository.KEY_BATTERY_WIDGET_BACKGROUND_ENABLED)
            ...
            try {
                val requestIntent = Intent("com.brittytino.airsync.action.REQUEST_MAC_BATTERY").apply {
                    setPackage("com.brittytino.airsync")
                }
                context.sendBroadcast(requestIntent, "com.brittytino.permission.ESSENTIALS_AIRSYNC_BRIDGE")
            } catch (e: Exception) {
