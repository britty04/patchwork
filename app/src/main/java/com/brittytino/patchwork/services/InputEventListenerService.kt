...
import com.brittytino.patchwork.input.InputDeviceScanner
import com.brittytino.patchwork.input.VolumeLongPressDetector
import com.brittytino.patchwork.input.VolumePressEvent
...
class InputEventListenerService : Service() {
    companion object {
        const val ACTION_VOLUME_LONG_PRESSED = "com.brittytino.patchwork.VOLUME_LONG_PRESSED"
    ...
    private fun startListening() {
        scope?.launch {
            if (!com.brittytino.patchwork.utils.ShellUtils.hasPermission(this@InputEventListenerService)) {
                    ...
                    if (event is VolumePressEvent.LongPress) {
                        ...
                        if (isTorchOn) {
                            ...
                            if (isAdjustEnabled || isGlobalEnabled) {
                                val action = if (event.direction == com.brittytino.patchwork.input.VolumeDirection.UP) 
                                    "com.brittytino.patchwork.ACTION_FLASHLIGHT_INCREASE" 
                                else 
                                    "com.brittytino.patchwork.ACTION_FLASHLIGHT_DECREASE"
                                sendBroadcast(Intent(action).setPackage(packageName))
                            } else {
                                val pm = getSystemService(android.content.Context.POWER_SERVICE) as android.os.PowerManager
                                val isScreenOn = pm.isInteractive
                                val suffix = if (isScreenOn) "_on" else "_off"
                                val key = if (event.direction == com.brittytino.patchwork.input.VolumeDirection.UP) "button_remap_vol_up_action$suffix" else "button_remap_vol_down_action$suffix"
                                val actionStr = prefs.getString(key, "None")
                                if (actionStr != "None") {
                                    val am = getSystemService(android.content.Context.AUDIO_SERVICE) as android.media.AudioManager
                                    val direction = if (event.direction == com.brittytino.patchwork.input.VolumeDirection.UP) 
                            ...
                            if (actionStr != "None") {
                                val am = getSystemService(android.content.Context.AUDIO_SERVICE) as android.media.AudioManager
                                val dirKey = if (event.direction == com.brittytino.patchwork.input.VolumeDirection.UP) 
                                    android.media.AudioManager.ADJUST_RAISE 
                                else 
                                    android.media.AudioManager.ADJUST_LOWER
